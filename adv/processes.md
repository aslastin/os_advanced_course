...
- В процессе наличествует один или несколько потоков исполнения. Изначально UNIX подобные системы начинали с единственного потока исполнения. На текущей момент, почти все системы поддерживают многопоточное исполнение. Чаще всего многотопоточное исполнение - несколько потоков исполнения, разделяющих одно и то же адресное пространство, вследствие чего способные взаимодействовать с одними и теми же данными.
...
- Стандартизированный интерфейс в ядро (__x86__: `syscall`/`sysenter`/`int`, __arm__: `svc`). Как мы уже поняли, процесс мало как может повлиять на ядро ОС, поэтому необходим какой-то интерфейс взамодействия. Так в __x86__ - есть специальные инструкции для взаимодействия с ОС. В самых разных ядрах используются разные механизмы для реализации системных вызовов. 
...
У процесса есть виртуальное адресное пространство. На рисунке изображен пример возможного layout виртуального адресного пространства и отображение на физическое адресное пространство. Здесь важно, что с точки зрения процесса существует набор областей, в которых находятся разные классы данных. В верху адресного пространства находится _стэк_, растет вниз, где-то находится _сегмент данных_ - данные, адресуемые абсолютным образом. 

_Стэк_ можно назвать ареной, в которой происходит аллокация короткоживущих локальных данных для той или иной функции, поэтому он не аллоцируется статически, а выделяется в зависимости от исполнения в процессе. 
...
Если рассмотреть реальный процессор и его реальную физическую память, там происходит адресация на линейном физическом устройстве (чем является современный массив данных), то в нем адресация происходит относительно какого-то нуля, страницы, и то, как проставлены стрелочки, то есть какой физической странице соответсвует та или иная логическая страница в том или ином процессе - это одна из важных функций ОС, менеджера виртуальной памяти, который создает для разных адресных пространств такого рода таблицы трансляции и при переключении контекстов (то есть исполняется один процесс, затем другой, как только такое переключение происходит) происходит правила трансляции виртуальных адресов в физические. 
...
_Стэк_ - кэш, арена, где хранятся локальные данные для функций. 
...
Первое, что мы смотрим в обработчике - какой номер системного вызова, попадает ли он в диапазон системных вызовов доступных системе. Если попадает, то передаем управление на вектор (функция ядра), которая реализует вызов `open`, дальше эта функция вылидирует корректность передаваемых значений.
...
Если все успешно - ядро открывает файл и запись полученного файлого дескриптора в таблице файловых дескрипторов процесса, после чего в каком-то регистре, записывает целое число, являющееся номером файлового дескриптора. 

Внутри ядра реализована нетривиальная машинерия обработки стандартиизированного интерфейса, которое предоставляет то или иное ядро. В разных ядрах ОС нумерация вызов и их семантика разная, именно это обеспечивает различие между ОС.
...
Если говорить высокоуровнево:  `fork` - создание нового адресного пространства с содержимым идентичным оригинальному адресному пространству, а `exec` - замещение содержимого текущего адресного пространства сегментами кода/данных/стэка, описанного в том или ином исполняемом файле. 
...
_Стэк_ - место, где сохраняются автоматические переменные программы. Стэк растет вниз из=за простоты аллокации. 
